...
interface FeaturesLayoutDialogProps {
  isOpen: boolean;
...
const layouts = [
  {
    ...
    gridStyles: "", // Empty gridStyles will trigger the centered grid layout
    preview: (
...
export function FeaturesLayoutDialog({ isOpen, onOpenChange, form }: FeaturesLayoutDialogProps) {
  const [selectedGridStyles, setSelectedGridStyles] = useState(form.getValues("features.gridStyles"));

  const handleSelectLayout = (layout: typeof layouts[0]) => {
    form.setValue("features.containerStyles", layout.containerStyles, { shouldDirty: true });
    form.setValue("features.gridStyles", layout.gridStyles, { shouldDirty: true });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl w-full">
        <DialogHeader>
          <DialogTitle>Choose a Features Layout</DialogTitle>
          <DialogDescription>
            Select a pre-defined layout for your features section. The content will adapt automatically.
          </DialogDescription>
        </DialogHeader>
