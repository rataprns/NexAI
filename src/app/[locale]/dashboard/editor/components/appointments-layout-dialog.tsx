...
interface AppointmentsLayoutDialogProps {
  isOpen: boolean;
...
const layouts = [
  {
    ...
    gridStyles: "grid gap-12 lg:grid-cols-2 lg:gap-24 items-center",
    preview: (
  ...
  },
  {
    ...
    containerStyles: "w-full py-20 md:py-28 lg:py-32 bg-background",
    gridStyles: "grid gap-12 lg:grid-cols-2 lg:gap-24 items-center [&>div:first-child]:order-last",
     preview: (
      <div className="grid grid-cols-2 gap-4 h-full">
        <Skeleton className="h-full w-full" />
        <div className="space-y-4">
...
export function AppointmentsLayoutDialog({ isOpen, onOpenChange, form }: AppointmentsLayoutDialogProps) {
  const [selectedGridStyles, setSelectedGridStyles] = useState(form.getValues("appointmentsSection.gridStyles"));

  const handleSelectLayout = (layout: typeof layouts[0]) => {
    form.setValue("appointmentsSection.containerStyles", layout.containerStyles, { shouldDirty: true });
    form.setValue("appointmentsSection.gridStyles", layout.gridStyles, { shouldDirty: true });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl w-full">
        <DialogHeader>
          <DialogTitle>Choose an Appointments Layout</DialogTitle>
          <DialogDescription>
            Select a pre-defined layout for your appointments section.
          </DialogDescription>
        </DialogHeader>
